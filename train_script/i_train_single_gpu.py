...
focal_roof = FocalLoss(alpha=0.25, gamma=2.0)  # Roof: 24% positive
...
def channel_specific_loss(pred, target, epoch, max_epoch):
    ...
    focal_weight = 0.7  # Give more importance to Focal Loss
    ...
    return focal_weight * f1 + (1 - focal_weight) * d1 # scalar tensor
...
def compute_batch_loss(outputs, masks, epoch, max_epoch, is_validation):
    ...
    if is_validation:
        ...
        boundary_weight = min(0.2, 0.012 * epoch)
        ssim_weight = min(0.25, 0.015 * epoch)  
    ...
    ssim_loss_val = 1 - SSIM(torch.sigmoid(main_out), masks, data_range=1.0,win_size=11, win_sigma=1.5, size_average=True)
    ...
    # --- Total Loss ---
    total_loss = (main_roof_loss) +  boundary_loss + (aux_roof_loss) + edge_reg + (ssim_weight * ssim_loss_val) # scalar tensor
...
# Training function
def train_epoch(model, train_loader, optimizer, scheduler, epoch, cfg):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        images = batch['image'].cuda() # Size: [B, 3, 512, 512]
        masks = batch['mask'].cuda() # Size: [B, 2, 512, 512]
...
def validation(model, val_loader, epoch, max_epoch):
    ...
    with torch.no_grad(): 
        for val_batch_idx, batch in enumerate(val_loader):
            ...
            # roof_mask = (torch.sigmoid(outputs_upsampled['main_out']) > 0.5).float()
            ...
            # Get binary predictions from the main output logits
            pred_probs = torch.sigmoid(outputs_upsampled['main_out'])
            pred_binary = (pred_probs > 0.5).cpu().numpy().astype(np.uint8)

            # Cleaned predictions will be stored here
            pred_cleaned = np.zeros_like(pred_binary)

            # Iterate over each image in the batch
            for i in range(pred_binary.shape[0]):
                # pred_binary[i, 0] is the roof channel for the i-th image
                labels = measure.label(pred_binary[i, 0], background=0)
                # Iterate over detected regions
                for region in measure.regionprops(labels):
                    # Remove small, noisy regions. Tune this area threshold.
                    if region.area > 100: # e.g., only keep regions with more than 100 pixels
                        # Add the valid region to the cleaned mask
                        for coords in region.coords:
                            pred_cleaned[i, 0, coords[0], coords[1]] = 1
            
            # Convert cleaned predictions back to a tensor for metric calculation
            # Use this 'pred_cleaned_tensor' instead of the raw output for calculate_metrics
            pred_cleaned_tensor = torch.from_numpy(pred_cleaned).float().cuda()
            
            # Calculate metrics on the CLEANED predictions
            roof_metrics = calculate_metrics(
                pred_cleaned_tensor, # Use the cleaned mask
                masks
            )
            ...
            if val_batch_idx % 2 == 0:
                print(f'Epoch: [{epoch}/{max_epoch-1}]  Val Batch: [{val_batch_idx}/{len(val_loader)-1}] '
                    f" Roof IoU: {roof_metrics['iou']:.4f}"
                    # f" Obs IoU: {obs_metrics['iou']:.4f}"
                    # f" Obs F1: {obs_metrics['f1']:.4f}")
                )
...
def update_learning_rates(optimizer, epoch):
    ...
    # Phase 2 (5-15): Boundary
        ...
        lrs = [0, 0, 0, base_lr * warmup_factor]  # [backbone, boundary, ocr, heads]
    
    # Phase 3 (15-25): Obstruction
    elif 15 <= epoch < 25:
        backbone_lr = min(1e-4, 1e-5*(epoch-4))
        boundary_lr = 1e-5 if epoch >= 10 else 0
        lrs = [backbone_lr, boundary_lr, 0, 2e-4]
...
def reinit_classifier(model):
    for m in [model.cls_head, model.aux_head[-1]]:
...
def main():
    ...
    # for name, param in model.named_parameters():
    #     param.requires_grad = False
    # # Unfreeze only the heads
    # for name, param in model.named_parameters():
    #     if any(k in name for k in ['cls_head', 'aux_head', 'ocr', 'boundary', 'edge']):
    #         param.requires_grad = True
    # # Re-create optimizer with only trainable parameters
    # trainable_params = filter(lambda p: p.requires_grad, model.parameters())
    # optimizer = torch.optim.AdamW(trainable_params, lr=1e-3, weight_decay=1e-4) # Use AdamW and a higher LR for heads
    # scheduler = None
    # current_stage = 1
    ...
    for epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCHES):
        ...
        trainable_paramss = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"Trainable params = {trainable_paramss}\n")
        ...
        # if epoch == 15 and current_stage == 1:
        #     print("--- Transitioning to Stage 2: Full Fine-tuning ---")
        #     current_stage = 2
        ...
        #     # 2. Re-create optimizer and scheduler for the new phase
        #     optimizer = torch.optim.AdamW(model.parameters(), lr=1e-5, weight_decay=1e-4)
        #     scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=10, eta_min=1e-6)
